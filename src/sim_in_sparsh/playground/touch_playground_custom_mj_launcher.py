...
MODEL_PATH = (
    f"{current_dir}/playground_data/touch_playground.xml"  # Update with your model path
)

CONTROL_INCREMENT = 0.01  # Amount to move joints per keypress
...
def main():
    ...
    while not glfw.window_should_close(window):
        ...
        object_id = mj.mj_name2id(model, mj.mjtObj.mjOBJ_GEOM, "digit")
        object_pos = data.geom_xpos[object_id]

        # Get position (3D) and rotation matrix (3x3)
        position = data.geom_xpos[object_id]  # Shape: (3,)
        rotation = data.geom_xmat[object_id].reshape(
            3, 3
        )  # Reshape flattened 9-element array to 3x3
